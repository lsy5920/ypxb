...
站点右下角提供了背景音乐开关，当前接入曲目：

- `清明雨上 - 许嵩`

音乐文件位置：

- `public/audio/qingming-yushang.mp3`

相关配置位于：

- `content/site.ts` 的 `ambientTrack`
- `content/site.ts` 的 `ambientHints`
- `components/ui/ambient-toggle.tsx`

如果你后续想换歌，直接替换 `public/audio/` 里的音频文件，并同步修改 `ambientTrack` 即可。
...
- `2026-03-13 18:09`：将分享海报重构为“人物主导型”版式，放大主像展示区域，修复原海报中主图被横向裁切后几乎不可见的问题。
- `2026-03-13 19:06`：放弃不稳定的复杂图文海报，重构为固定像素画布的纯文字装饰版；分离预览节点与导出节点，重点保证主标题、文案块与二维码完整导出。
- `2026-03-13 19:37`：接入真实背景音乐《清明雨上》，用真实音频播放器替换原有合成提示音；补充音乐资源说明与后续替换方式。
